...
const ReviewColumn = ({reviews,className,reviewClassName,msPerPixel = 0}:{reviews:string[],className?:string,reviewClassName?:(reviewIndex:number)=>string,msPerPixel?:number}) => {
        ...
        <div ref={containerRef} className="relative -mx-4 mt-16 grid h-[49rem] max-h-[150vh] grid-cols-1 items-start gap-8 overflow-hidden px-4 sm:mt-20 md:grid-cols-2 lg:grid-cols-3">
            {isInView ? (
                <>
                    ...
                    reviewClassName={(reviewIndex) => reviewIndex >= column2.length ? "ld:hidden" : ""}
                    msPerPixel={15}
                    />
                    <ReviewColumn
